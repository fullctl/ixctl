...
from fullctl.django.decorators import load_instance
...
@reversion.register
class MRTGConfig(HandleRefModel):

    """
    Describes a configuration for a MRTG instance
    """

    ix = models.ForeignKey(
        InternetExchange,
        on_delete=models.CASCADE,
        related_name="mrtgconfig_set",
    )
    ...
    host_name_str = "{snmp_community}@{switch_name}:{port}:{timeout}:{retries}:{backoff}:{snmp_version}"

    def port_speed_in_bytes(port_name:str):
        if port_name.startswith("Gigabit") or port_name.startswith("ge-"):
            return 1_000_000_000 // 8
        if port_name.startswith("TenGi") or port_name.startswith("xe-"):
            return 10_000_000_000 // 8
        if port_name.startswith("TwentyFiveGi"):
            return 25_000_000_000 // 8
        if port_name.startswith("FortyGi"):
            return 40_000_000_000 // 8
        if port_name.startswith("HundredGi") or port_name.startswith("et-"):
            return 100_000_000_000 // 8
    ...
    def generate(self):
        ...
        # for switch aggregate sub-template
        switch_aggregates = []
        for device in devices:
            switch_aggregates.append({
                "switch_name": device.name,
                "ports": [port.name for port in ports_by_device[device.name]],
                "aggregate_bandwidth": sum([self.port_speed_in_bytes(port.name) for port in ports_by_device[device.name]]),
            })
        ...
        mrtg_vars = {
            ...
            "ix_all_ports": ix_all_ports,
            "switch_aggregates": switch_aggregates,
            "member_ports": member_ports,
        }

        self.body = json.dumps(mrtg_vars)
        self.save()
